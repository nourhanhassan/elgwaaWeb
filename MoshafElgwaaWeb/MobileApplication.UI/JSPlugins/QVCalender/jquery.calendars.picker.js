...
(function ($) { // Hide scope, no $ conflict
    ...
    function CalendarsPicker() {
        this._defaults = {
            ...
            yearRange: '-120:+30', // Range of years to show in drop-down: 'any' for direct text entry
        ...
        this.regional[''] = { // Default regional settings
            ...
            prevStatus: 'مشاهدة الشهر السابق', // Status text for the previous month command
            prevJumpText: '&lt;&lt;', // Text for the previous year command
            prevJumpStatus: 'مشاهدة السنة السابقة', // Status text for the previous year command
            nextText: 'التالى&gt;', // Text for the next month command
            nextStatus: 'مشاهدة الشهر التالى', // Status text for the next month command
            nextJumpText: '&gt;&gt;', // Text for the next year command
            nextJumpStatus: 'مشاهدة السنة التإلية', // Status text for the next year command
            currentText: 'الجارى', // Text for the current month command
            currentStatus: 'مشاهدة الشهر الجارى', // Status text for the current month command
            todayText: 'اليوم', // Text for the today's month command
            todayStatus: 'مشاهدة تاريخ اليوم', // Status text for the today's month command
            clearText: 'حذف', // Text for the clear command
            clearStatus: 'حذف التاريخ', // Status text for the clear command
            closeText: 'إغلاق', // Text for the close command
            closeStatus: 'إغلاق التقويم', // Status text for the close command
            yearStatus: 'تعديل السنة', // Status text for year selection
            monthStatus: 'تعديل الشهر', // Status text for month selection
            weekText: 'Wk', // Text for week of the year column header
            weekStatus: 'الاسبوع فى السنة', // Status text for week of the year column header
            dayStatus: 'اختر DD, M d, yyyy', // Status text for selectable days
            defaultStatus: 'اختر تاريخ ', // Status text shown by default
    ...
    $.extend(CalendarsPicker.prototype, {
        ...
        defaultRenderer: {
            ...
            picker: '<div class="calendars dblcal-hijri">' +
            '<div class="calendars-nav">{link:prev}{link:today}{link:next}</div>{months}' +
            '{popup:start}<div class="calendars-ctrl" style="width:100%;clear:both;">{link:clear}{link:close}</div>{popup:end}' +
        ...
        _showPlugin: function (target) {
            ...
            if (inst) {
                ...
                if (inst.div) {  // updated by QV Team
        ...
        _hidePlugin: function (target, immediate) {
            ...
            if (inst && inst == plugin.curInst) {
                ...
                if (inst.div != null) { // updated by QV Team
                    inst.div.stop();
                    if ($.effects && $.effects[showAnim]) {
